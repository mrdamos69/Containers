...
class s21_list : public abstract_containers_list<T> {
 public:
    ...
    // внутренний класс ListConstIterator<T>определяет тип константы для итерации по контейнеру
    ...
    template<typename ... Arg>
    void emplace_back(T value, Arg&&...args);  // добавляет новые элементы в конец контейнера
    void emplace_back() {}

    template<typename ... Arg>
    void emplace_front(T value, Arg&&...args);  // добавляет новые элементы в верхнюю часть контейнера
    void emplace_front() {}
