...
class s21_set : public abstract_containers_tree<T> {
 ...
 public:
    ...
    как внутренний итератор подкласса дерева определяет тип для итерации по контейнеру*/
    ...
    s21_set() : abstract_containers_tree<T>(), element(nullptr), back_elem(nullptr) {}
    ...
    size_type max_size() override;  // возвращает максимально возможное количество элементов
    ...
    std::pair<iterator, bool> emplace() { return std::pair<iterator, bool>(this->end(), true); }
